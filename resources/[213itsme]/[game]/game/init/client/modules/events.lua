...
AddStateBagChangeHandler("group", nil, function(bagName, _, value)
    if value == nil then
        return
    end

    local myServerId = GetPlayerServerId(PlayerId())
    if not myServerId or myServerId == 0 then
        return
    end

    if bagName == ("player:%s"):format(myServerId) then
        ESX.SetPlayerData("group", value)
    end
end)
