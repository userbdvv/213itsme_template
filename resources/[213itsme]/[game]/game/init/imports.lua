...
if not IsDuplicityVersion() then -- Only register this event for the client
    AddEventHandler("esx:setPlayerData", function(key, val, last)
        if GetInvokingResource() == "game" then
    ...
    for i = 1, #external do
        ...
        local file = LoadResourceFile("game", path)
        if file then
            local fn, err = load(file, ('@@game/%s'):format(path))
    ...
    local function createStaticPlayer(src)
        return setmetatable({ src = src }, {
            __index = function(self, method)
                return function(...)
                    return exports.game:RunStaticPlayerMethod(self.src, method, ...)
                end
            end
        })
    end
...
if GetResourceState("ox_lib") == "missing" then
    ...
    local function getModuleInfo(modName)
        ...
        while true do
            ...
            if resource and not src:find('^@@game/init/imports') then
