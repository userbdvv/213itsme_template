...
local newPlayer = "INSERT INTO `users` SET `uniqueId` = ?, `accounts` = ?, `identifier` = ?, `ssn` = ?, `group` = ?"
...
loadPlayer = loadPlayer .. ", uniqueId FROM `users` WHERE identifier = ?"
...
local function createESXPlayer(identifier, playerId, data)
    ...
    local uniqueId = MySQL.scalar.await(
        "SELECT IFNULL(MAX(uniqueId), 0) + 1 FROM users"
    )
...
if Config.Multichar then
    AddEventHandler("esx:onPlayerJoined", function(src, char, data)
        while not Core.JobsLoaded or not Core.Jobs2Loaded do
            Wait(50)
        end
...
function loadESXPlayer(identifier, playerId, isNew)
    ...
    local job2, grade2 = result.job2 or "unemployed2", tostring(result.job2_grade or 0)

    if not ESX.DoesJobExist(job, grade) then
        print(("[^3WARNING^7] Invalid job %s:%s for %s, fallback unemployed"):format(job, grade, identifier))
    ...
    if not ESX.DoesJob2Exist(job2, grade2) then
        print(("[^3WARNING^7] Invalid job2 %s:%s for %s, fallback unemployed2"):format(job2, grade2, identifier))
    ...
    if result.group then
        userData.group = (result.group == "superadmin") and "admin" or result.group
    else
        userData.group = "user"
    end
    ...
    userData.coords = json.decode(result.position)
        or Config.DefaultSpawns[ESX.Math.Random(1, #Config.DefaultSpawns)]

    userData.skin = (result.skin and result.skin ~= "")
        and json.decode(result.skin)
        or { sex = 0 }

    userData.metadata = (result.metadata and result.metadata ~= "")
        and json.decode(result.metadata)
        or {}
    ...
    xPlayer.uniqueId = userData.uniqueId
    Player(playerId).state:set('uniqueId', xPlayer.uniqueId, true)
    ...
    if result.firstname and result.firstname ~= "" then
        ...
        if result.dateofbirth then xPlayer.set("dateofbirth", result.dateofbirth) end
        if result.sex then xPlayer.set("sex", result.sex) end
        if result.height then xPlayer.set("height", result.height) end
    end
    ...
    print(('[^2INFO^0] Player "%s" connected | ID %s | UID %s'):format(
        xPlayer.getName(),
        playerId,
        xPlayer.getUniqueId()
    ))
...
AddEventHandler('esx:playerLoaded', function(source, xPlayer)
    TriggerClientEvent('gamemode:discord:updateplayer', source, GetNumPlayerIndices())
end)


CreateThread(function()
    while (true) do
        TriggerClientEvent('gamemode:discord:updateplayer', -1, GetNumPlayerIndices())
