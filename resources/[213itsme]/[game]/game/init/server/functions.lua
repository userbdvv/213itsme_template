...
function ESX.RefreshJobs2()
    ...
    if jobGrades then
        for _, v in ipairs(jobGrades) do
            if Jobs2[v.job_name] then
                ...
                print(('[^3WARNING^7] Ignoring job2 grades for ^5"%s"^0 due to missing job2'):format(v.job_name))
            end
        end
    end
    ...
    if ESX.Table.SizeOf(Jobs2) == 0 then
        local fallbackGrades = {
            ["0"] = { grade = 0, name = "unemployed2", label = "Sans gang", salary = 0, skin_male = "{}", skin_female = "{}" },
        }
...
---@param grade string
---@return boolean
function ESX.DoesJob2Exist(job, grade)
    while not Core.Jobs2Loaded do
        Citizen.Wait(200)
    end

    return (ESX.Jobs2[job] and ESX.Jobs2[job].grades[tostring(grade)] ~= nil) or false
end
