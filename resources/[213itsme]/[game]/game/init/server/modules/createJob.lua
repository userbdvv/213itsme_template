...
function ESX.CreateJob(name, label, grades)
    ...
    success = exports.game:transaction_async(queries)
...
--- Create Secondary Job (job2) at Runtime
--- @param name string
--- @param label string
--- @param grades table
function ESX.CreateJob2(name, label, grades)
    ...
    local currentJobExist = doesJob2AndGradesExist(name, grades)
    ...
    for _, grade in pairs(grades) do
        queries[#queries + 1] = {
            query = 'INSERT INTO job2_grades (job_name, grade, name, label, salary, skin_male, skin_female) VALUES (?, ?, ?, ?, ?, ?, ?)',
            values = {
                name,
                grade.grade,
                grade.name,
